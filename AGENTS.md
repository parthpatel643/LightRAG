...
- `build_graph.py`, `query_graph.py`: standalone CLI helpers for ingesting/querying a local graph with `--profile`/`--timing` flags (see *Profiling & Performance* below).
...
- **api/**: FastAPI service (`lightrag_server.py`) with REST endpoints and Ollama-compatible API; routers under `routers/` (including this fork's sequencing routes), static Swagger assets, packaged WebUI output, and Gunicorn launcher.
- **evaluation/**: This fork's evaluation framework (semantic equivalence, LLM-based temporal metrics).
...
- **Neptune+OpenSearch** (this fork): dual-write graph storage with OpenSearch-backed search.
...
# With sequencing metadata (this fork): sequence_index/effective_date/doc_type
await rag.ainsert("Text", sequence_index=1, effective_date="2026-01-01", doc_type="policy")
```
...
See `env.example` for comprehensive template. This fork loads `.env` from multiple candidate paths (see `lightrag/api/config.py`) and supports auth token renewal.
...
- This repo is a fork of `HKUDS/LightRAG`. Target `HKUDS/LightRAG:main` (upstream) when creating PRs, not the fork's own repo, unless the change is specific to this fork's temporal/versioning features.
- PR descriptions should include: summary, motivation, linked issues if applicable, what's changed, what's broken and how it works.
- Verify `ruff check .`, `python -m pytest tests`, and affected Bun commands succeed before requesting review; note the runs in the PR text.
