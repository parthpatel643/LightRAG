...
# Temporal versioning: best-effort ``effective_date`` extraction from a raw
# document's own content when the caller didn't supply one explicitly (e.g.
# a contract/amendment body containing a line like "Effective Date: 2024-03-01").
_EFFECTIVE_DATE_RE = re.compile(
    r"Effective\s*Date\s*[:：][^\d\n]{0,10}(\d{4}-\d{2}-\d{2})", re.IGNORECASE
)
...
class _PipelineMixin:
    ...
    async def apipeline_enqueue_documents(
        ...
        Args:
            ...
            metadata: optional temporal versioning metadata, a single dict
                (broadcast) or a ``list[dict]`` aligned with ``input``. If
                every entry already carries a ``sequence_index`` (e.g. from
                the batch_upload_sequenced endpoint), those values are
                preserved; otherwise ``sequence_index`` is auto-assigned via
                an atomic batch allocation. ``effective_date`` is additionally
                auto-extracted from RAW document content when not provided
                (pattern: ``Effective Date: YYYY-MM-DD``). Stored in
                ``full_docs[doc_id]['metadata']`` and mirrored to
                ``doc_status.metadata`` (carried across every state
                transition, see ``_DOC_STATUS_METADATA_CARRY_OVER_KEYS``).
        ...
        # Temporal versioning: auto-assign or preserve sequence_index metadata.
        # If every document's metadata already carries a sequence_index (e.g.
        # from the batch_upload_sequenced endpoint), those values are
        # preserved; otherwise sequence_index is auto-assigned via an atomic
        # batch allocation to avoid gaps on partial failure (Issue #16 fix).
        ...
        if has_provided_sequence_indices:
        ...
        def _add_content(
            ...
            # Temporal versioning metadata (sequence_index / effective_date /
            # doc_type) resolved up-front for the whole batch. When the
            # caller didn't supply an effective_date, try to extract one
            # from the document's own raw content before falling back to
            # "unknown".
            doc_metadata = dict(resolved_metadata[index])
            if (
                doc_format == FULL_DOCS_FORMAT_RAW
                and doc_metadata.get("effective_date") == "unknown"
            ):
                if match := _EFFECTIVE_DATE_RE.search(content or ""):
                    doc_metadata["effective_date"] = match.group(1)
            content_data["metadata"] = doc_metadata
        ...
        def _initial_doc_status(content_data: dict[str, Any]) -> dict[str, Any]:
            ...
            # Temporal versioning metadata must survive into doc_status so
            # it's visible without a full_docs lookup, and so it carries
            # across every PENDING -> ... -> PROCESSED/FAILED transition
            # via _DOC_STATUS_METADATA_CARRY_OVER_KEYS.
        ...
        async with enqueue_serialize_lock:
            ...
            for doc_id in new_docs.keys():
                ...
                # Temporal versioning metadata (sequence_index / effective_date
                # / doc_type) is always populated by ``_add_content`` at
                # enqueue time so it's persisted unconditionally.
                if contents[doc_id].get("metadata") is not None:
                    full_docs_data[doc_id]["metadata"] = contents[doc_id]["metadata"]
