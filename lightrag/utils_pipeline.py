...
def build_chunks_dict_from_chunking_result(
    ...
    *,
    ...
    ``doc_metadata`` optionally carries temporal versioning fields
    (``sequence_index`` / ``effective_date`` / ``doc_type``) resolved at
    enqueue time (see ``lightrag.pipeline.apipeline_enqueue_documents``);
    when present, they're stamped onto every chunk so per-chunk retrieval
    can filter/sort by document version without a full_docs lookup.
    """
    ...
    for dp in chunking_result:
        ...
        if doc_metadata:
            for key in ("sequence_index", "effective_date", "doc_type"):
                if key in doc_metadata:
                    chunks[chunk_key][key] = doc_metadata[key]
    return chunks
...
_DOC_STATUS_METADATA_CARRY_OVER_KEYS: tuple[str, ...] = (
    ...
    # Temporal versioning metadata (set at enqueue time by
    # LightRAG.apipeline_enqueue_documents); must survive every
    # PENDING -> ... -> PROCESSED/FAILED status transition.
    "sequence_index",
    "effective_date",
    "doc_type",
)
